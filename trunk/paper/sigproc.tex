...
We measured the performance of \textit{SGLd} and \textit{SGLs} against other streaming partitioning algorithms on our set of real-world graphs. These experiments were run by partitioning the input graphs on a set of target partitions of $k=4$ with the memory limitation in the range of $30\%, 50\%$ and $ 80\%$, as shown in Table 2, Table 3 and Table 4. It can be observed that the algorithms of 'Hash' and 'B' perform the best, since they don't compute the global measure such as degree
\begin{table*}[!hpp]
\centering
